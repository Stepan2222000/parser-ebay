...
5. **Перевыдача.** Processing с `dispatched_at < now() −
   ...
   старый всё-таки допишет — запись идемпотентна). Это единственный возврат
   брошенных задач: при аварии (kill -9, смерть сервера — в отличие от
   штатного SIGTERM-дренажа) задачи ждут именно этот таймаут;
   heartbeat'ов воркеров осознанно нет.
...
Диагностика — централизованно из БД: `status`, журнал `tasks` (растущие
`attempts` = кто-то умирает), `parse_errors` с HTML. Логи контейнеров
(stdout / `docker logs`) — только для разбора конкретного случая; сборщика
логов до платформы нет.
...
- **Единица мощности — воркер** (контейнер); платформа будет управлять их
  числом (ресурсный ориентир: ~1 CPU, ~1.2 GiB на воркера).
...
- **Дашборд и сборщик логов** — принесёт платформа (текущая диагностика —
  §8).
